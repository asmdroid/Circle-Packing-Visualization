...
The Circle Packing Visualization (CPV) presents two levels of detail:

 - State level : This is the first level where the groups of variables are depicted as circles. The size of the groups depend on the number of variables within.
...
Both states and variables are shown with a color code. The color code of the CPV is based on user defined values we call weights. Weights are defined taking into account the following:
